...
export default function Options({ message }) {
  ...
  const handleSubmit = e => {
    ...
    if (!message.conversationId) return alert("id not found")
    let participantUser = message?.receiver?.email !== myInfo?.email ? message.receiver : message?.sender

    editConversation({
      id: message.conversationId,
      sender: myInfo?.email,
      data: {
        participants: `${myInfo?.email}-${participantUser?.email}`,
        users: [myInfo, participantUser],
